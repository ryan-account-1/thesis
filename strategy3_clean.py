...
print("\n" + "-" * 70)
print("[A.1] Descriptive statistics of RR and AbRR")
print("-" * 70)

desc_cols = ["RR", "AbRR", "AbRR_z", "vix", "SPX_ret"]
desc = analysis_df[desc_cols].describe(percentiles=[0.01, 0.05, 0.25, 0.50, 0.75, 0.95, 0.99])
desc.loc["skew"] = analysis_df[desc_cols].skew()
desc.loc["kurtosis"] = analysis_df[desc_cols].kurtosis()
desc = desc.round(5)
desc.to_csv(os.path.join(APPENDIX_DIR, "A1_descriptive_stats.csv"))
print(desc.to_string())

#AbRR regression output 

print("\n" + "-" * 70)
print("[A.2] Full-sample AbRR regression (reference)")
print("-" * 70)

reg_df = rr_df.dropna(subset=["RR", "vix", "Month"]).copy()
X_full = pd.concat([
    pd.DataFrame({"const": 1.0, "vix": reg_df["vix"].astype(float).values}, index=reg_df.index),
    pd.get_dummies(reg_df["Month"].astype(int), prefix="m", drop_first=True).astype(float)
], axis=1)
y_full = reg_df["RR"].astype(float)
model_full = sm.OLS(y_full, X_full).fit()

coef_table = pd.DataFrame({
    "Variable": model_full.params.index,
    "Coefficient": model_full.params.values.round(6),
    "Std Error": model_full.bse.values.round(6),
    "t-stat": model_full.tvalues.values.round(3),
    "p-value": model_full.pvalues.values.round(4),
})
coef_table.to_csv(os.path.join(APPENDIX_DIR, "A2_regression_coefficients.csv"), index=False)

reg_meta = pd.DataFrame({
    "Metric": ["N observations", "R-squared", "Adjusted R-squared",
               "F-statistic", "F p-value", "Residual std error"],
    "Value": [int(model_full.nobs),
              round(model_full.rsquared, 4),
              round(model_full.rsquared_adj, 4),
              round(model_full.fvalue, 2),
              round(model_full.f_pvalue, 6),
              round(np.sqrt(model_full.mse_resid), 5)],
})
reg_meta.to_csv(os.path.join(APPENDIX_DIR, "A2_regression_summary.csv"), index=False)

print(coef_table.to_string(index=False))
print("\nRegression diagnostics:")
print(reg_meta.to_string(index=False))

# List of signal events

print("\n" + "-" * 70)
print("[A.3] Signal events — ALL 67")
print("-" * 70)

signal_table = analysis_df[analysis_df["Event_lag1"] == True][
    ["date", "close", "SPX_ret", "vix", "RR", "AbRR", "AbRR_z"]
].copy()
signal_table.columns = ["Signal_Date", "SPX_Close", "SPX_Return",
                        "VIX", "RR", "AbRR", "AbRR_z"]
signal_table["Signal_Date"] = pd.to_datetime(signal_table["Signal_Date"]).dt.strftime("%Y-%m-%d")
signal_table["SPX_Return"] = (signal_table["SPX_Return"] * 100).round(2)
signal_table[["SPX_Close", "VIX"]] = signal_table[["SPX_Close", "VIX"]].round(2)
signal_table[["RR", "AbRR", "AbRR_z"]] = signal_table[["RR", "AbRR", "AbRR_z"]].round(5)
signal_table = signal_table.reset_index(drop=True)
signal_table.index = signal_table.index + 1
signal_table.index.name = "#"
signal_table.to_csv(os.path.join(APPENDIX_DIR, "A3_signal_events.csv"))
print(signal_table.to_string())
print(f"\nTotal: {len(signal_table)} signal events")

# List of trades

print("\n" + "-" * 70)
print("[A.4] Executed trades — ALL 34")
print("-" * 70)

trade_table = results_df[[
    "Entry_Date", "Exit_Date", "Days_Held", "Exit_Reason",
    "VIX_Entry", "RR_Entry", "RR_Exit", "RR_Change",
    "IV_Spread_Entry", "TC_Total_$", "Net_PnL_$"
]].copy()
trade_table["Entry_Date"] = pd.to_datetime(trade_table["Entry_Date"]).dt.strftime("%Y-%m-%d")
trade_table["Exit_Date"] = pd.to_datetime(trade_table["Exit_Date"]).dt.strftime("%Y-%m-%d")
trade_table["VIX_Entry"] = trade_table["VIX_Entry"].round(2)
trade_table[["RR_Entry", "RR_Exit", "RR_Change", "IV_Spread_Entry"]] = (
    trade_table[["RR_Entry", "RR_Exit", "RR_Change", "IV_Spread_Entry"]].round(5)
)
trade_table[["TC_Total_$", "Net_PnL_$"]] = trade_table[["TC_Total_$", "Net_PnL_$"]].round(2)
trade_table = trade_table.reset_index(drop=True)
trade_table.index = trade_table.index + 1
trade_table.index.name = "#"
trade_table.to_csv(os.path.join(APPENDIX_DIR, "A4_executed_trades.csv"))
print(trade_table.to_string())
print(f"\nTotal: {len(trade_table)} executed trades")

#Threshold sensitivity

print("\n" + "-" * 70)
print("[A.5] Threshold sensitivity")
print("-" * 70)

robust_rows = []
for q in [0.05, 0.075, 0.10, 0.125, 0.15]:
    for z in [-0.5, -0.75, -1.0, -1.25]:
        n_sig = ((analysis_df["ret_pct_rank"] <= q) &
                 (analysis_df["AbRR_z"] <= z)).sum()
        robust_rows.append({"Return_Percentile": q, "AbRR_Z_Threshold": z,
                            "N_Signals": int(n_sig)})
robust = pd.DataFrame(robust_rows)
robust.to_csv(os.path.join(APPENDIX_DIR, "A5_threshold_sensitivity.csv"), index=False)
print(robust.to_string(index=False))

print("\n" + "=" * 70)
print(f"All appendix files also saved to /{APPENDIX_DIR}/")
print("=" * 70)